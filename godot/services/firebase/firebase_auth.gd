...
func _post(url: String, body: Dictionary) -> Dictionary:
	...
	var error_code := "AUTH_FAILED"

	if parsed.has("error") and parsed["error"].has("message"):
		error_code = str(parsed["error"]["message"])
...
func _save_auth(data: Dictionary) -> void:
	...
	save.set_value("auth", "id_token", id_token)
	save.set_value("auth", "refresh_token", refresh_token)
	save.set_value("auth", "uid", uid)
	save.set_value("auth", "email", email)
	save.save("user://auth.cfg")


func load_session() -> bool:
	var save := ConfigFile.new()

	if save.load("user://auth.cfg") != OK:
		return false

	id_token = str(save.get_value("auth", "id_token", ""))
	refresh_token = str(save.get_value("auth", "refresh_token", ""))
	uid = str(save.get_value("auth", "uid", ""))
	email = str(save.get_value("auth", "email", ""))
